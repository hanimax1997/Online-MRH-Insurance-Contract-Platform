...
outputGarantieFlotte(params: any[], idDevis: string): void {
  ...
  const widths   = headers.map(() => colWidth);      // tableau de % pour pdfMake
  ...
  /* lignes données */
  const headersSansPrime = headers.slice(1, -1);     // on retire N° ordre & prime nette

  params.forEach((vehicule, idx) => {
    const row: any[] = [];

    /* N° ordre ----------------------------------------------------------- */
    row.push({
      text      : vehicule.idRisque ?? '-',
      alignment : 'center',
      fontSize  : 8
    });

    /* Garanties ---------------------------------------------------------- */
    headersSansPrime.forEach(lib => {
      const g    = vehicule.garantieList?.find((x: any) => x.description === lib);
      const text = g?.prime
        ? Number(g.prime).toLocaleString('fr-FR', { minimumFractionDigits: 2 })
        : '-';
      row.push({ text, alignment:'center', fontSize:8 });
    });

    /* Prime nette véhicule ---------------------------------------------- */
    const primeNet = vehicule.primeList
                     ?.find((p: any) => p?.typePrime?.code === 'CP101')
                     ?.prime ?? null;

    row.push({
      text      : primeNet
                    ? Number(primeNet).toLocaleString('fr-FR',
                        { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' DZD'
                    : '-',
      alignment : 'center',
      fontSize  : 8
    });
  ...
  /* 3.  Définition pdfMake – TYPÉE ! ------------------------------------- */
  const docDefinitionPrimesGaranties: TDocumentDefinitions = {
    watermark   : { text: '', color: 'blue', opacity: 0.1 },
    pageMargins : [35, 110, 35, 90] as [number, number, number, number],
    pageOrientation: 'landscape',

    header: (currentPage: number) => ({
      text   : `Devis N° : ${idDevis}`,
      style  : 'sectionHeader',
      margin : [35, currentPage === 1 ? 30 : 10, 35, 0] as [number, number, number, number]
    }),
  ...
  pdfMake.createPdf(docDefinitionPrimesGaranties).download('Devis_Garanties');
    ...
    /* ------------------------------------------------------------------ */
    /*  Version “mono-produit” MRH – lisible, courte et à jour            */
    /* ------------------------------------------------------------------ */
    /* ------------------------------------------------------------------ */
    /*  Génération du PDF MRH                                             */
    /* ------------------------------------------------------------------ */
    /* ------------------------------------------------------------------ */
    /*  Génération du PDF MRH                                             */
    /* ------------------------------------------------------------------ */
    /**
     * Génération du PDF Devis – Produit 96 (MRH uniquement)
     * @param devis  Le JSON complet retourné par le back après `createDevis`
     */
    /* -------------------------------------------------------------------------- */
    /*  Génération du PDF – Produit 96 (MRH)                                      */
    /* -------------------------------------------------------------------------- */
    ...
    * -------------------------------------------------------------------------- */
    async generatePdf(devis: any) {  
        ...
        const logo = await getBase64ImageFromURL(this.http, 'assets/images/AXA_LOGO.png');
        ...
        columns:[
            {
                ...
                table: {
                        ...
                        body: [
                            [
                                {
                                    image: await getBase64ImageFromURL(this.http, 'assets/images/AXA_LOGO.png'),
                                    alignment: 'left',
                                    width: 40,
                                    height: 40,
                                },
        ...
        async outputDevisFlotte(devis: any){  
    ...
    /* -------------------------------------------------------------------------
    * Helper conservé depuis le service original
    * ----------------------------------------------------------------------- */
    private formatReponse(risque: any): string {
    if (!risque) { return ''; }
    const type = (risque as any)?.typeChamp?.description;
    if (type === 'Liste of values') { return (risque as any)?.reponse?.idParamReponse?.description || ''; }
    if (type === 'From Table')      { return (risque as any)?.reponse?.description || ''; }
    return (risque as any)?.reponse?.valeur ?? '';
    }
    ...
    mailDevis(idDevis: number, token: string): Observable<string> {
